...
```bash
conda create -n git_dbx_cicd python=3.9
conda activate git_dbx_cicd
```
...
```
This command will create your personal repository under `/Repos/<username>/git_dbx_cicd`.
...
- To trigger the release pipeline, get the current version from the `git_dbx_cicd/__init__.py` file and tag the current code version:
```
