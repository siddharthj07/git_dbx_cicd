...
def extract_name_level_anchors(block_lines: List[str], child_indent: int) -> List[str]:
    anchors: List[str] = []
    pattern = re.compile(rf"^\s{{{child_indent}}}<<:\s*\*[A-Za-z0-9_-]+\s*$")
...
def replace_pause_status_vars(section: str) -> str:
    section = re.sub(
...
def simplify_schedule_cron(schedule: str) -> str:
    function_names = set(re.findall(r"custom\.([A-Za-z0-9_]+)\(", schedule))
    if function_names - {"cron_schedule"}:
        return schedule

    pattern = re.compile(
        r"""^(\s*quartz_cron_expression:\s*)\{\{\s*custom\.cron_schedule\(\s*(['"])(.*?)\2.*\)\s*\}\}\s*$""",
        re.MULTILINE,
    )
    return pattern.sub(r'\1"\3"', schedule)
...
def build_job_yaml(
    ...
    for anchor in name_level_anchors:
        lines.append(reindent_section(anchor, old_indent, 6))
...
def parse_jobs(source_text: str) -> Tuple[Dict[str, Dict[str, str]], Dict[str, str]]:
    ...
    for block in split_job_blocks(source_text):
        ...
        name_level_anchors = extract_name_level_anchors(lines, child_indent)
        ...
        if schedule:
            schedule = replace_pause_status_vars(schedule)
            schedule = simplify_schedule_cron(schedule)
